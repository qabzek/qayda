...
ҰБТ алдындағы навигатор — 11-сынып оқушыларына профильдік пәндер комбинациясы бойынша мамандық, ЖОО, грант, өту балы, ауыл квотасы, кәсіп және жалақы туралы ақпарат беретін веб-сервис.

Стиль: **digital brutalism × editorial × Gen-Z**. Тіл: тек қазақ тілі (интерфейс).

## ⚠️ Деректер туралы маңызды ескерту

Бұл MVP-де деректердің үш деңгейі бар:

- ✅ **Ресми дерек** (`sourceId: "official-mon"`) — ҚР Ғылым және жоғары білім министрлігінің 2026 жылғы шекті балл кестесінен алынған (`src/data/minimumScores.ts`, 1009 жазба, 94 ЖОО × 41 мамандық). Бұл — қатысу үшін қажетті **шекті балл**, нақты өту балы емес.
- ✅ **Univision.kz / Taylor.kz дерегі** (`sourceId: "univision"` / `"taylor-raem"`) — тәуелсіз агрегаторлардан алынған грант саны, өту балы (B057 үшін) және IT жалақысы.
- ⚠️ **Демо дерек** (`sourceId: "demo"`) — әлі тексерілмеген, "Үлгі дерек" бейджісімен көрсетіледі.

Барлық 158 ГОП коды мен 94 ЖОО атауы — ресми министрлік кестесінен, сондықтан **шатаспайды**. Бірақ грант саны, нақты өту балы, ауыл квотасы және жалақы деректерінің көбі әлі демо күйінде.

**Production-ке шығармас бұрын:**
1. `src/data/grants.ts`, `src/data/passingScores.ts` — қалған демо сандарды testcenter.kz-тен нақтылау
2. `src/data/salaries.ts` — Enbek.kz немесе басқа ашық нарық дерегімен толықтыру
3. `src/data/sources.ts` — әр жаңа деректің `Source` жазбасын қосу және сол `sourceId`-ды тиісті файлда пайдалану (сонда `Үлгі дерек` бейджісі жоғалып, нақты дереккөз көрсетіледі)
4. Университеттердің `city` өрісі — көбіне "Басқа" күйінде (тек атауда қала аты тікелей кездескенде ғана автоматты түрде толтырылды, қалғаны болжанбады)
